...
namespace Kungumam.Services
{
    public class WrapperService : IWrapperService
    {
        ...
        public DataTable GetEditWrapper(string id)
        {
            ...
            SvSql = "SELECT book_id,wrapper_id,book_wrapper,CONVERT(varchar, book_wrapper.issue_dt, 106) AS AddedDateFormatted,dt FROM book_wrapper where wrapper_id = '" + id + "'";
        ...
        public DataTable GetAllWrapper(string strStatus)
        {
            ...
            {
                SvSql = "select book.book_name,wrapper_id,book_wrapper,CONVERT(varchar, book_wrapper.issue_dt, 106) AS AddedDateFormatted from book_wrapper LEFT OUTER JOIN book ON book.book_id=book_wrapper.book_id where book_wrapper.flag='n' ORDER BY wrapper_id DESC ";
            }
            DataTable dtt = new DataTable();
            SqlDataAdapter adapter = new SqlDataAdapter(SvSql, _connectionString);
            SqlCommandBuilder builder = new SqlCommandBuilder(adapter);
